<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>Italva FOOD (Exemplo)</title>
	<meta name="viewport"
	      content="width=device-width, initial-scale=1, shrink-to-fit=no, user-scalable=no, viewport-fit=cover"/>
	<meta name="format-detection" content="telephone=no"/>
	<meta http-equiv="X-UA-Compatible" content="IE=edge"/>
	<meta name="MobileOptimized" content="176"/>
	<meta name="HandheldFriendly" content="True"/>
	<meta name="robots" content="noindex, nofollow"/>
	<script src="https://tg.dev/js/telegram-web-app.js?7"></script>
	<script>
        function setThemeClass() {
            document.documentElement.className = Telegram.WebApp.colorScheme;
        }

        Telegram.WebApp.onEvent('themeChanged', setThemeClass);
        setThemeClass();
	</script>
	<link href="css/cafe.css" rel="stylesheet">
</head>
<body>
<section class="cafe-page cafe-items">
	<div class="cafe-item js-item" data-item-id="1" data-item-price="4990">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Burger.tgs">
				<img src="./img/Burger_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Hamburger">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Hamburger</span>
			<span class="cafe-item-price">R$15</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="2" data-item-price="1490">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Fries.tgs">
				<img src="./img/Fries_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Batata-Frita">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Batata-Frita</span>
			<span class="cafe-item-price">R$5</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="3" data-item-price="3490">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Hotdog.tgs">
				<img src="./img/Hotdog_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Hot-Dog">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Hot-Dog</span>
			<span class="cafe-item-price">R$10</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="4" data-item-price="3990">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Tako.tgs">
				<img src="./img/Tako_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Taco">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Taco</span>
			<span class="cafe-item-price">R$25</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="5" data-item-price="7990">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Pizza.tgs">
				<img src="./img/Pizza_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Pizza">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Pizza</span>
			<span class="cafe-item-price">R$40</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="6" data-item-price="1490">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Donut.tgs?1">
				<img src="./img/Donut_148.png?1"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Rosquinha">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Rosquinha</span>
			<span class="cafe-item-price">R$5</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="7" data-item-price="1990">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Popcorn.tgs">
				<img src="./img/Popcorn_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Pipoca">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Pipoca</span>
			<span class="cafe-item-price">R$8</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="8" data-item-price="1490">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Coke.tgs">
				<img src="./img/Coke_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Coca-Cola">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Coca-Cola</span>
			<span class="cafe-item-price">R$8</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="9" data-item-price="10990">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Cake.tgs">
				<img src="./img/Cake_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Bolo">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Bolo</span>
			<span class="cafe-item-price">R$30</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="10" data-item-price="5990">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Icecream.tgs">
				<img src="./img/Icecream_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Sorvete">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Sorvete</span>
			<span class="cafe-item-price">R$6</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="11" data-item-price="3990">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Cookie.tgs">
				<img src="./img/Cookie_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Cookie">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Cookie</span>
			<span class="cafe-item-price">R$4</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item js-item" data-item-id="12" data-item-price="7990">
		<div class="cafe-item-counter js-item-counter">1</div>
		<div class="cafe-item-photo">
			<picture class="cafe-item-lottie js-item-lottie">
				<source type="application/x-tgsticker" srcset="./img/tgs/Flan.tgs">
				<img src="./img/Flan_148.png"
				     style="background-image: url('data:image/svg+xml;base64,');"
				     alt="Pudim">
			</picture>
		</div>
		<div class="cafe-item-label">
			<span class="cafe-item-title">Pudim</span>
			<span class="cafe-item-price">R$20</span>
		</div>
		<div class="cafe-item-buttons">
			<button class="cafe-item-decr-button js-item-decr-btn button-item ripple-handler">
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
			<button class="cafe-item-incr-button js-item-incr-btn button-item ripple-handler">
				<span class="button-item-label">Adicionar</span>
				<span class="ripple-mask"><span class="ripple"></span></span>
			</button>
		</div>
	</div>
	<div class="cafe-item-shadow"></div>
	<div class="cafe-item-shadow"></div>
	<div class="cafe-item-shadow"></div>
	<div class="cafe-item-shadow"></div>
</section>
<section class="cafe-page cafe-order-overview">
	<div class="cafe-block">
		<div class="cafe-order-header-wrap">
			<h2 class="cafe-order-header">Sua Compra</h2>
			<span class="cafe-order-edit js-order-edit">Editar</span>
		</div>
		<div class="cafe-order-items">
			<div class="cafe-order-item js-order-item" data-item-id="1">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Burger.tgs">
						<img src="./img/Burger_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Hamburger">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Hamburger <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">HAMBURGER PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$4.99</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="2">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Fries.tgs">
						<img src="./img/Fries_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Batata-Frita">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Batata Frita <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">POTATO PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$5</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="3">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Hotdog.tgs">
						<img src="./img/Hotdog_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Hot-Dog">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Hot-Dog <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">HOTDOG PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$10</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="4">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Tako.tgs">
						<img src="./img/Tako_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Taco">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Taco <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">TACO PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$25</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="5">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Pizza.tgs">
						<img src="./img/Pizza_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Pizza">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Pizza <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">PIZZA PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$40</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="6">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Donut.tgs?1">
						<img src="./img/Donut_148.png?1"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Rosquinha">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Rosquinha <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">DONUT_PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$5</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="7">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Popcorn.tgs">
						<img src="./img/Popcorn_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Pipoca">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Pipoca <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">POPCORN PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$8</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="8">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Coke.tgs">
						<img src="./img/Coke_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Coca-Cola">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Coca-Cola <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">CEIRA PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$8</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="9">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Cake.tgs">
						<img src="./img/Cake_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Bolo">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Bolo <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">CAKE PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$30</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="10">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Icecream.tgs">
						<img src="./img/Icecream_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Sorvete">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Sorvete <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">ICECREAM PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$6</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="11">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Cookie.tgs">
						<img src="./img/Cookie_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Cookie">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Cookie <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">COOKIE PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$4</div>
			</div>
			<div class="cafe-order-item js-order-item" data-item-id="12">
				<div class="cafe-order-item-photo">
					<picture class="cafe-item-lottie js-item-lottie">
						<source type="application/x-tgsticker" srcset="./img/tgs/Flan.tgs">
						<img src="./img/Flan_148.png"
						     style="background-image: url('data:image/svg+xml;base64,');"
						     alt="Pudim">
					</picture>
				</div>
				<div class="cafe-order-item-label">
					<div class="cafe-order-item-title">Pudim <span class="cafe-order-item-counter"><span
									class="js-order-item-counter">1</span>x</span></div>
					<div class="cafe-order-item-description">PUDIM PLACEHOLDER</div>
				</div>
				<div class="cafe-order-item-price js-order-item-price">R$20</div>
			</div>
		</div>
	</div>
	<div class="cafe-text-field-wrap">
		<textarea class="cafe-text-field js-order-comment-field cafe-block" rows="1"
		          placeholder="Adicionar comentário…"></textarea>
		<div class="cafe-text-field-hint">
			Qualquer pedido especial, detalhes, etc.
		</div>
	</div>
</section>
<div class="cafe-status-wrap">
	<div class="cafe-status js-status"></div>
</div>
<script src="https://tg.dev/js/jquery.min.js"></script>
<script src="https://tg.dev/js/tgsticker.js?27"></script>
<script src="js/cafe.js?version=<?php echo uniqid() ?>"></script>
<script>
    Cafe.init({
        "apiUrl": "<?php echo R$_ENV['REMOTE_PATH'] . '/telegram' ?>",
        "userId": 0,
        "userHash": null
    });
</script>
</body>
</html>